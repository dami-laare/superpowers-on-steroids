...
2. **Explore project context** — check files, docs, recent commits (share findings in the dispatch prompt, or tell the Brainstormer to explore itself). In fast mode (`<SUPERPOWERS_CONFIG>` with `mode: fast`), dispatch `superpowers-on-steroids:caveman-investigator` on the standard tier (see subagent-driven-development Model Selection) to gather this instead of exploring inline.
